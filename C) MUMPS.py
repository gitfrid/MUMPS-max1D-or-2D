...
filename_plot_text = "Dowhy causal estimate on mean vac coverage and cases Mumps"
...
log_file_path = fr"C:\MUMPS-max1D-or-2D\C) {filename_plot_text} valid years for dowhy calc {year_range_text}.txt"
...
if len(causal_effects) > 0:
    ...
    upper_percentile_causal = np.percentile(causal_effects, 98.5)
    upper_percentile_vac = np.percentile(mean_vac, 98.5)
    upper_percentile_cases = np.percentile(mean_cases, 98.5)
    
    # Calculate the 5th percentile (lower bound for clipping negative values)
    lower_percentile_causal = np.percentile(causal_effects, 0.95)
    lower_percentile_vac = np.percentile(mean_vac, 0.95)
    lower_percentile_cases = np.percentile(mean_cases, 0.95)
    
    # Apply Winsorization: Clip extreme high values (above the 99.2 percentile) and extreme low values (below the 5th percentile)
    causal_effects_winsorized = [
        max(min(effect, upper_percentile_causal), lower_percentile_causal) for effect in causal_effects
    ]
    mean_vac_winsorized = [
        max(min(vac, upper_percentile_vac), lower_percentile_vac) for vac in mean_vac
    ]
    mean_cases_winsorized = [
        max(min(cases, upper_percentile_cases), lower_percentile_cases) for cases in mean_cases
    ]
    ...
    fig.update_layout(
        title=f"Dowhy Causal Effect estimation max(1st or 2nd Dose) Vaccination Coverage on Reported Cases/1M ({year_range_text})",
        xaxis_title="Country",
        showlegend=True,
    ...
    # Save the plot
    fig.write_html(fr"C:\MUMPS-max1D-or-2D\C) {filename_plot_text} {year_range_text}.html")

    # Show the plot
    fig.show()

    print(fr"C:\MUMPS-max1D-or-2D\C) {filename_plot_text} {year_range_text}.html has been saved.")
else:
